# LSAI Protocol v1.1 — Language Server for AI

> Revision: v1.1 (2026-02-07)
...
LSAI is a protocol for AI consumption of compiler-semantic intelligence. It defines **11 tools**, their **data contracts**, and a **plugin capability system** — but NOT a specific output format. Format is an implementation detail handled by pluggable formatters.
...
5. **Format Agnostic** — The protocol defines what data each tool returns (the contract). How that data is serialized (JSON, pipe-delimited text, language-native syntax) is an implementation choice. See [Output Format Profiles](#output-format-profiles).
...
## What Actually Matters (Empirical)

Based on 6+ months of production usage comparing different output formats across Claude Opus 3.5/4.6 and OpenAI Codex:
...
| **Output format style** | NEGLIGIBLE | Pipe-delimited, colon-separated, JSON — all work with zero LLM friction |

The format itself is a commodity. Any compact format works. LLMs decode pipe-delimited (`method|internal|16,48`), colon-separated (`file:16 method`), and JSON equally well without any legend or explanation.
...
Tools use short names: `search`, `info`, `usages`, `callers`, `callees`, `hierarchy`, `impact`, `rename`, `diagnostics`, `outline`, `deps`.
...
```json
{"count":4,"symbols":{
  "IDocumentService":"interface|Contracts/IDocumentService.cs|8",
  "DocumentService":"class|Services/DocumentService.cs|14"}}
```
...
**Example output (compact JSON):**
```json
{"DocumentService":"class|public sealed|Services/DocumentService.cs|14",
 "namespace":"EFine.Services.Domain",
 "base":"BaseService",
 "implements":["IDocumentService","IDisposable"],
 "members":"3P 7M 2F",
...
**Example output (compact JSON):**
```json
{"count":5,"files":3,"usages":{
  "Controllers/DocController.cs":["28|private readonly IDocumentService _service;","45|public DocController(IDocumentService service)"],
  "Services/DocumentService.cs":["14|public class DocumentService : IDocumentService"]}}
```
...
down:
  IDocumentService  Contracts/IDocumentService.cs:8
    DocumentService  Services/DocumentService.cs:14
    CachedDocumentService  Services/CachedDocumentService.cs:9
    MockDocumentService  Tests/Mocks/MockDocumentService.cs:6
[3 implementations]
```
...
| `code` | yes | Diagnostic code (e.g., `CS0103`) |
...
**Example output (compact text):**
```
Services/DocumentService.cs:32 error CS0103: '_repo' does not exist in the current context
Config/Startup.cs:44 error CS0121: ambiguous call between 'Configure(A)' and 'Configure(B)'
[2 errors]
```

This format is identical to `dotnet build` / `gcc` / `rustc` / `tsc` compiler output.
...
| `kind` | yes | `class`, `struct`, `interface`, `method`, `property`, `field`, `event`, `constructor` |
...
```json
{"count":"1C 6m","outline":{
  "DocumentService":"class|public sealed|:BaseService,IDocumentService|14",
  "DocumentService.members":{
    "_repository":"field|private|IRepository|16",
    ".ctor":"method|internal|(IRepository,IValidator,ILogger)|20",
    "Process":"method|public|async Task<DocumentDto>(int,CancellationToken)|28",
    "Validate":"method|public|bool(Document)|45",
    "IsInitialized":"property|public|bool{get}|68"}}}
```

Note how compact JSON includes return types and parameter types in the value — this is the key data enrichment over VS-MCP compact format.
...
| `protocol` | yes | Protocol version (e.g., `LSAI/1.1`) |
...
protocol: LSAI/1.1
plugins:
  roslyn-csharp  tier 3  [C#, F#]  11 tools
  pyright        tier 2  [Python]   9 tools
workspaces:
...
|---------|-------|----------|
| **CompactJson** | Pipe-delimited values in JSON structure | Production default. Proven zero-friction with LLMs |
| **CompactText** | Grep-like / compiler-like plain text | Protocol compliance, human-readable logs |
...
- Include **code context** in usages
- Use **full words** for kinds (`class`, not `C`; `error`, not `E`)

### Why Format Doesn't Matter

Empirical testing across 6+ months with Claude Opus 3.5/4.6 and OpenAI Codex showed:
...
- Do NOT return raw LSP JSON — 80%+ token waste
...
| What | v1.0 | v1.1 | Why |
|------|------|------|-----|
| **Format philosophy** | Pipe-delimited with abbreviations | Format-agnostic (data contracts) | Format doesn't matter. Data richness does |
| **Kind indicators** | `C`, `I`, `M`, `P`, `F` (collisions: E=enum AND error) | Full words: `class`, `interface`, `method` | Zero ambiguity, negligible cost |
| **Severity** | `E`, `W`, `I`, `H` | `error`, `warning`, `info`, `hint` | No collisions |
| **Tool names** | `searchSymbols`, `symbolInfo`, `inheritance` | `search`, `info`, `hierarchy` | Shorter |
| **Outline** | Kind prefix + name + line | **Full signature** + line | Eliminates follow-up Reads |
| **Usages** | File + line + col + context | File-grouped + line + context | Cleaner grouping |
| **Plugin manifest** | 15 fields | 6 fields | Runtime state removed |
| **Output format** | Single prescribed format | Multiple profiles supported | Implementations choose |
| **obj/ filtering** | Not specified | MUST filter auto-generated | Data quality |
| **deps** | Not specified clearly | MUST be project-to-project | Not NuGet package list |
...
- **LSAI/1.1** — This document. Format-agnostic, data contracts, enriched fields.
